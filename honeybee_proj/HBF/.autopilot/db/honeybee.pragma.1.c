...
_ssdm_SpecFuncInstantiation(num, "");
# 83 "src/honeybee.c"

    Dout_t collisions[4] = {0, 0, 0, 0};
...
# 104 "src/honeybee.c"
        ...
        if (num==0) {
            if (pointOnSegment(POI, edge)) {
                collisions[i] = (collisions[i] | (or << shiftAmount((int) POI.x, (int) POI.y, i)));
                collisions[i] = (collisions[i] | (or << shiftAmount((int) POI.x, (int) POI.y, i-1)));
            }
        }
            ...
            if (pointOnSegment(POI, _edge)) {
                collisions[i] = (collisions[i] | (or << shiftAmount((int) POI.x, i, (int) POI.z)));
                collisions[i] = (collisions[i] | (or << shiftAmount((int) POI.x, i-1, (int) POI.z)));
            }
        }
            ...
            if (pointOnSegment(POI, _edge)) {
                collisions[i] = (collisions[i] | (or << shiftAmount(i, (int) POI.y, (int) POI.z)));
                collisions[i] = (collisions[i] | (or << shiftAmount(i - 1, (int) POI.y, (int) POI.z)));
            }
        }
    }
